...
function activateMode(index) {
  const tab = Array.from(document.querySelectorAll(".mode-tab"))[index];
  if (!tab) return;
  tab.click();
...
function findSectionByText(text) {
  return Array.from(document.querySelectorAll("h2, h3, .card-title, button"))
...
function Sidebar({ mode, completion, metrics, activeItem, onNavigate }) {
  ...
  return (
    <aside className="estimate-sidebar no-print" aria-label="見積管理メニュー">
      <div className="estimate-sidebar-brand"><span className="estimate-sidebar-logo" aria-hidden="true">ME</span><div><strong>Mitsumori</strong><span>見積管理</span></div></div>
...
function Header() {
  return <header className="app-hero app-hero-v2 no-print"><div className="app-hero-copy"><div className="app-hero-meta"><span className="app-product-kicker">CUSTOM PROGRAM ESTIMATE</span><span className="app-context-badge">内部業務用</span></div><h1>短期留学プログラム見積管理</h1><p>案件の基礎情報から費用入力、最終確認、Excel保存までを一つの画面で管理します。</p></div><div className="app-header-summary" aria-label="アプリの管理方針"><span>計算方式</span><strong>係数方式・積み上げ方式</strong><small>既存の計算ロジックとExcel形式を維持</small></div></header>;
}
...
function StatusBar({ mode, completion, metrics }) {
  ...
  return (
    <section className="workflow-status workflow-status-v2 no-print" aria-label="作業状況">
      ...
      <MetricCard icon="見" label="見積金額合計" value={metrics.estimateTotal} note="既存画面の計算結果を表示" />
...
export default function ProfessionalUiShell() {
  ...
  useEffect(() => {
    ...
    refresh();
    return () => { observer.disconnect(); document.removeEventListener("input", refresh, true); document.removeEventListener("change", refresh, true); };
  }, []);
  ...
  useEffect(() => {
    ...
    const sidebarMount = document.createElement("div"); sidebarMount.setAttribute("data-estimate-sidebar", ""); document.body.prepend(sidebarMount);
    const headerMount = document.createElement("div"); headerMount.setAttribute("data-professional-header", ""); container.insertBefore(headerMount, originalTitle);
    const statusMount = document.createElement("div"); statusMount.setAttribute("data-workflow-status", ""); container.insertBefore(statusMount, tabs);
    setSidebarTarget(sidebarMount); setHeaderTarget(headerMount); setStatusTarget(statusMount);
