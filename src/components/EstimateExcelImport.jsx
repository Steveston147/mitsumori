...
function parseInlineWorksheet(xmlText) {
  ...
  if (documentXml.querySelector("parsererror")) {
    throw new Error("TransferDataシートを解析できませんでした。");
  }
...
function parseBackupPayload(arrayBuffer) {
  ...
  const hasReactState = Object.keys(payload.storage).some((key) =>
    key.startsWith(ESTIMATE_STATE_PREFIX)
  );
  if (!hasReactState) {
    throw new Error(
      "このExcelにはReact State復元データがありません。最新版の画面から再度エクスポートしてください。"
    );
  }
...
export default function EstimateExcelImport() {
  ...
  async function handleFile(event) {
    ...
    try {
      ...
      console.error(error);
      const message =
        error instanceof Error ? error.message : "Excelの読込に失敗しました。";
      window.dispatchEvent(
        new CustomEvent("mitsumori-workspace-message", { detail: { message } })
      );
    }
  }

  return (
    <div data-estimate-excel-import hidden>
      <input
        ref={inputRef}
        type="file"
