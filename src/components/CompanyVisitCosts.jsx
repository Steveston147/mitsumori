...
function getVisibleCount(value) {
  ...
  return Math.min(count, MAX_VISIT_DETAILS);
}
...
export default function CompanyVisitCosts({ count, visits, onChange }) {
  ...
  return (
    <div className="col-12">
      ...
      {countIsInvalid && <div className="warn">企業訪問回数は0以上の整数で入力してください。</div>}
      {!countIsInvalid && requestedCount > MAX_VISIT_DETAILS && <div className="warn">詳細入力は最大{MAX_VISIT_DETAILS}回分まで表示しています。</div>}
      {!countIsInvalid && visibleCount === 0 && <div className="cost-empty">企業訪問が1回以上の場合に、訪問別の経費入力欄を表示します。</div>}
      {visibleVisits.map((visit, visitIndex) => {
        ...
        return <section className="visit-card" key={visitIndex}>
          ...
          <div className="cost-table-wrap"><table className="table visit-cost-table"><thead><tr><th>経費項目</th><th>単価</th><th>数量</th><th>税区分</th><th>消費税</th><th>小計</th></tr></thead><tbody>
            {visit.lines.map((line, lineIndex) => {
              ...
              return <tr key={line.key} className={!lineResult.ok ? "cost-line-invalid" : undefined}>
                ...
                <td><input type="number" min="1" step="1" value={line.quantity} onChange={(e) => updateLine(visitIndex, lineIndex, "quantity", e.target.value)} /></td>
          ...
          {!summary.ok && <div className="warn">単価は0円以上、数量は1以上の整数で入力してください。</div>}
        </section>;
      })}
